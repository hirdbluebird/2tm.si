...
$output .=' <div class="item-description" id="tes44"><h5><a href="'. get_permalink().'">'.get_the_title().'</a></h5>';
...
$output .=' <div class="item-description" id="tes45"><h5><a href="'. get_permalink().'">'.get_the_title().'</a></h5><p>'.$short_excerpt.'</p></div></div>';
    endwhile;  // close the Loop
    endif;
    wp_reset_query();
...
function mag_testimonials($atts,$content ) {
 ...
 $output = '<div class="headline no-margin"><div class="wasH3" id="tes35">'.$title.'</div></div> <div class="testimonials-carousel" data-autorotate="3000"><ul class="carousel">'.do_shortcode( $content ).'</ul></div>';
 return $output;
}
...
function pp_pricing_table($atts, $content) {
    ...
    $output .= '<h3>'.$header.'</h3>';
    $output .= '<h4 id="tes36"><span class="price">'.$price.'</span>';
